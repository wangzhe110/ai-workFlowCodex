...
def test_confirmed_storyboard_generates_versioned_images() -> None:
    with TestClient(app) as c:
        p = c.post("/api/v1/projects", json={"title":"图片测试"}).json()["id"]; c.post(f"/api/v1/projects/{p}/source-video", files={"file":("x.mp4",real_video_bytes(),"video/mp4")}); c.post(f"/api/v1/projects/{p}/analysis-runs"); c.post(f"/api/v1/projects/{p}/topic-generation-runs")
